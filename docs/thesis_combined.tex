...
診斷問卷以 L1=1、L2=2、L3=3 的加權規則計算掌握度，並以護欄規則抑制偶然高分。後端流程包括：填空複核、Prompt A 報告生成、欄位級 merge/fallback、topic\_tag 校正，確保輸出可解析且可用。診斷輸出的建議起點難度（\texttt{recommended\_start\_level}）會在後續批次報告中作為 \texttt{baselineLevel} 候選來源之一，與上一批 \texttt{recommendedLevel} 共同形成下一輪編排基準。
...
若未達目標數量，系統在上限輪次內補生；超限後回退題庫題以保證可用性。L4 模式採 \textbf{soft-ai-priority}：提高 AI 題比重與生成配額，但不硬性要求 5/5 全 AI；若生成可用題不足，允許題庫題補位，並回傳 \texttt{l4AiCoverage} 與 \texttt{l4FallbackRate} 供量化分析。
...
\subsection{掌握報告與正規化}
\texttt{/api/mastery/report} 將診斷、練習事件與批次報告組裝後交由模型生成 \texttt{MasteryReportV2}，伺服端再進行欄位正規化（難度、topic 分數、順序去重、敘事長度）並提供 fallback，確保儀表板穩定渲染。實作上，下一批練習優先採用最近一份批次報告的 \texttt{recommendedLevel} 作為 \texttt{baselineLevel}，若不存在才回退診斷起點，形成跨批次的可追蹤難度遞移鏈。

\section{Prompt 工程與可靠性設計}
\subsection{Prompt 分工策略}
本研究採「低溫結構化、高溫對話化」原則：0.10--0.25 用於排序/驗題/JSON 任務，0.30--0.35 用於生成與敘事，0.45 用於導師對話與提示改寫。此分工可同時兼顧可解析性與語言自然度。對批次升降級而言，AI 僅提供 \texttt{difficulty\_advice}/\texttt{recommended\_level} 建議，最終決策由伺服端規則引擎依 \texttt{baselineLevel -> advice -> level} 單步轉移（最多 $\pm 1$ 級）完成，屬「演算法主控、AI 輔助」設計。
...
\section{系統驗證與評估設計}
\subsection{功能驗證結果}
以工程驗證角度，系統已完成診斷、練習、對話、批次報告、掌握儀表板與降級機制的端到端串接。包含「無 embedding」、「RPC 未建立」、「模型回傳不合規 JSON」等失效情境，皆可透過 fallback 維持可用回覆。另已驗證批次升降級綁定：\texttt{up} 必升、\texttt{down} 必降、\texttt{keep} 維持；\texttt{difficultyAdvice} 與 \texttt{recommendedLevel} 不再出現方向矛盾。

\subsection{量化指標}
\begin{itemize}[leftmargin=2em]
  \item 升降級一致率：\texttt{advice\_level\_consistency\_rate}
  \item L4 AI 覆蓋率：\texttt{l4\_ai\_coverage}
  \item L4 補位率：\texttt{l4\_fallback\_rate}
  \item 生成題有效率：\texttt{validatedCount/generatedCount}
  \item 驗題淘汰率：\texttt{validation\_reject\_rate}
...
\begin{enumerate}[leftmargin=2em]
  ...
  \item 外部效度有限（單一場域與樣本）。
  \item L4 為高 AI 比重而非硬性全 AI；當生成可用題不足時會回退題庫補位，故需以 coverage/fallback 指標判讀實際強化程度。
\end{enumerate}
