...
export default function DiagnosticPage() {
  ...
  const persistAndGoReport = async (nextTimes: Record<number, number>) => {
    ...
    try {
      const r = await fetch("/api/diagnostic/analyze", {
        ...
        body: JSON.stringify({ results, totalTimeSeconds }),
      });
      if (r.ok) {
        const data = (await r.json()) as { report?: ClientReport };
    ...
    if (typeof window !== "undefined") {
      sessionStorage.setItem(DIAGNOSTIC_STORAGE_KEY, JSON.stringify({ payload, report }));
    }
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-6">
      ...
      <article className="rounded-2xl border border-zinc-200/80 bg-white p-6 shadow-sm dark:border-zinc-800 dark:bg-zinc-900/70">
        <div className="flex flex-wrap gap-2 text-xs font-medium">
          ...
          <span className="rounded-full bg-zinc-50 px-2.5 py-0.5 text-zinc-500 dark:bg-zinc-600 dark:text-zinc-300">
            {q.topicTag}
          </span>
        </div>
              ...
              return (
                <li key={opt.key}>
                  <label
                    className={
                      selected
                        ? "flex cursor-pointer items-center gap-3 rounded-xl border-2 border-teal-600 bg-teal-50/80 p-3 dark:border-teal-500 dark:bg-teal-950/40"
                        : "flex cursor-pointer items-center gap-3 rounded-xl border border-zinc-200 p-3 transition hover:border-zinc-300 hover:bg-zinc-50 dark:border-zinc-700 dark:hover:border-zinc-600 dark:hover:bg-zinc-800/50"
                    }
                  >
                    <input
                      type="radio"
        ...
        {q.kind === "fill" && (
          <div className="mt-6">
            <textarea
              className="w-full rounded-xl border border-zinc-200 bg-zinc-50/80 px-3 py-2.5 text-zinc-900 placeholder:text-zinc-400 focus:border-teal-500 focus:outline-none focus:ring-2 focus:ring-teal-500/20 dark:border-zinc-700 dark:bg-zinc-950 dark:text-zinc-100"
              rows={q.fillRows ?? 2}
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <button
          ...
          onClick={goPrev}
          disabled={index === 0}
          className="rounded-xl border border-zinc-300 bg-white px-4 py-2.5 text-sm font-medium text-zinc-800 disabled:cursor-not-allowed disabled:opacity-40 dark:border-zinc-600 dark:bg-zinc-900 dark:text-zinc-200"
        >
          上一題
        </button>
        <div className="flex gap-2">
          <Link
            href="/"
            className="rounded-xl px-4 py-2.5 text-sm text-zinc-600 underline-offset-4 hover:underline dark:text-zinc-400"
          >
            回到首頁
          </Link>
          <button
            ...
            className="rounded-xl bg-teal-700 px-5 py-2.5 text-sm font-semibold text-white shadow-sm transition hover:bg-teal-800 disabled:cursor-not-allowed disabled:bg-zinc-300 dark:bg-teal-600 dark:hover:bg-teal-500 dark:disabled:bg-zinc-700"
          >
            {finishing
              ? "分析中…"
              : index < total - 1
