...
===== 上一輪建議等級（baseline）=====
${JSON.stringify(input.baselineLevel ?? null)}
...
5. recommended_level：依表現建議下一組練習等級；若表現明顯超出 L3，可給 L4（此欄位作為建議，最終等級由伺服端規則綁定決策）。
6. 若 baseline 有值，請以 baseline 為基準判斷上升/維持/下降，不要跳級過大。
7. 嚴禁輸出 JSON 以外內容。`;
}
